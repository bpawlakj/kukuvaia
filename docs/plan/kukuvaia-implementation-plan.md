...
- [ ] `ChatClientConfig` — ChatClient bean with advisor chain
- [ ] `MemoryConfig` — JdbcChatMemoryRepository + MessageWindowChatMemory
- [ ] `ToolRegistryConfig` — Tool catalog from @McpTool beans
- [ ] `PostgresTools` — 5 data tools
- [ ] `MongoTools` — 3 data tools
- [ ] `AuthorsuiteTools` — 2 data tools
- [ ] `AgentService` — Chat orchestration
- [ ] `CommandRouter` — Slash commands + agent loop routing
- [ ] `PersonaService` — Persona YAML loading + tool filtering
- [ ] `OutputBlock` — Sealed interface hierarchy (4 types)
- [ ] `ChatController` — SSE streaming endpoint
- [ ] `SessionController` — Session CRUD
- [ ] `CommandController` — Command dispatch endpoint
- [ ] Slash commands: `/help`, `/validate`, `/search`, `/model`, `/login`
- [ ] `ToolRegistry` — Resolve @McpTool beans by name
- [ ] Complete SubAgentFactory TODO (line 99)
...
- [ ] `DocumentTools` — 5 file operation tools with sandbox
- [ ] `FileStateCache` — LRU read deduplication
- [ ] Tool result persistence — large results → disk
- [ ] Workspace configuration (`kukuvaia.workspace.*`)
- [ ] `MemoryTools` — 4 explicit memory tools
- [ ] `MemoryRepository` — CRUD + full-text search
- [ ] `PersistentMemoryAdvisor` — auto-inject at conversation start
- [ ] `MemoryExtractionAdvisor` — async post-response extraction
- [ ] V2 migration: `agent_memory` table
- [ ] `PlanningTools` — 3 planning tools
- [ ] V3 migration: `agent_plans` table
- [ ] `VerificationTools` — 3 verification tools
- [ ] `SystemPromptBuilder` — static/dynamic boundary
- [ ] Verification mandate in system prompt
- [ ] Extended `OutputBlock` types (Plan, StepProgress, Verification, Source, Memory, Metadata)
- [ ] `ToolHookDispatcher` — pre/post tool events
- [ ] Increase `maxToolRounds` to 20
- [ ] Integration test: create plan → execute steps → verify → report
...
- [ ] `LayeredCompactionMemory` — snip → auto-compact → collapse
- [ ] `IntentDetectionAdvisor` — classify → route
- [ ] `WorkflowRegistry` + built-in workflows (summarize, validate, compare, export)
- [ ] CommandRouter integration with WorkflowRegistry (3-tier routing)
- [ ] Tool concurrency metadata (readOnly flag)
- [ ] Parallel tool execution for read-only tools
- [ ] `TokenBudgetAdvisor` — per-turn budget tracking + warning injection
- [ ] Permission modes per persona (auto/default/bypass)
- [ ] `CommandTools` — allowlisted subprocess execution
- [ ] Command allowlist configuration (`kukuvaia.commands.allowed.*`)
- [ ] `ExtensionLoader` — .kukuvaia/ discovery
- [ ] `RulesLoader` — rules → system prompt
- [ ] `SkillsLoader` — skills → on-demand activation
- [ ] `UserCommandLoader` — user commands → registry
- [ ] Integration test: full workflow — intent → plan → execute → verify → memory extraction
