...
- **`plan.md`** is the living plan. Edited in place when scope changes. If the plan was produced by `/discover` or `/research`, reference citations (`> Ref: docs/reference/<file>`) are already embedded in the plan body and travel through to tasks via `/atomize`.
