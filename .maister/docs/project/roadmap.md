...
| **P24 Context Compaction (token-threshold, unified)** | 3A/3B | **Phases A–E all shipped** (Phase E pin mechanism in place; P08 cache-marker reader TODO) | replaces `TokenBudgetAdvisor` (✅ removed); supersedes P04 (Phase D fills the orphan `conversations.summary` column) |
...
*Last updated: 2026-05-18 (P24 Phases C + E shipped, closing out the strategy ladder. Phase C: new `ToolCompactionRegistry` + `ToolCompactSummary` functional interface — tool-owning teams register a `(args, response) → summary` hook keyed by tool name; old tool responses (outside `keep-last-turns`) are summarised via the registry, with a configurable default-elision fallback when no hook is registered. Built-in summaries: `introspect_section_schema`, `get_outline_sections`, `get_outline_snapshot`. Phase E: same registry exposes `pin(toolName)` — pinned tools' pairs survive Phase A drops and their bodies survive Phase C summaries; a one-time-per-model warning logs when an active model has no `contextWindow` in the registry. New strategy ladder: B(retry collapse) → B(duplicate fold) → C(per-tool summary) → A(hard-cap drop) → D(LLM summarisation). Counters now include `toolResponsesSummarised`. 17 new tests for Phase C, 5 new for Phase E; total 117 advisor tests green. P08 prompt-cache marker reader noted as TODO but mechanism in place. Earlier today: Phases B + D landed; Phase A on 2026-05-15.)*
