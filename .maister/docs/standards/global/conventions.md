...
Every Java class gets a Javadoc comment describing its purpose. Reference security classes (guards, filters, sanitizers) for traceability in security-critical code.

### Single Source Theme File
`kukuvaia-theme.yaml` is the single source of truth for visual identity across all clients. No hardcoded color values or style tokens outside this file.

### HTTP/SSE API Contract
All client-server communication uses HTTP/SSE with JSON payloads. No alternative transport protocols.
